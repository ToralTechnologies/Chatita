...
function domainTheme(type?: string, severity?: string) {
  ...
  if (severity && SEVERITY_THEME[severity]) return { ...SEVERITY_THEME[severity], kicker: severity === 'warning' ? 'Something to notice' : severity === 'success' ? 'Going well' : 'Insight' };
...
export default function InsightsPage() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      if (correlationRes.ok) setCorrelation(await correlationRes.json());
      if (insightsRes.ok) {
  ...
  // Build scatter dots from trendData (each point: { time: ISO, value: number })
  ...
  // All cards (patterns + ai insights)
  ...
  const hasData = stats && (stats.averageGlucose > 0 || (correlation?.mealsTracked ?? 0) > 0);
  ...
  if (loading) {
    return (
      <div className="min-h-screen mobile-page-pb" style={{ background: '#F7EFE1' }}>
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {[1, 2, 3].map(i => (
  ...
  if (!hasData) {
    return (
      <div className="min-h-screen mobile-page-pb" style={{ background: '#F7EFE1' }}>
        <PageHeader period={period} setPeriod={setPeriod} correlation={correlation} />
  ...
  return (
    <div className="min-h-screen mobile-page-pb" style={{ background: '#F7EFE1' }}>
      ...
      <div style={{ maxWidth: '980px', margin: '0 auto', padding: '20px 20px 48px' }}>

        {/* ── Daily glucose pattern (scatter) ── */}
        <div style={{ background: '#FFFDF9', borderRadius: '22px', border: '1px solid rgba(1,35,116,0.07)', padding: '24px 26px', boxShadow: '0 14px 30px -24px rgba(1,35,116,.3)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
            <div>
              <h2 className="font-serif-italic" style={{ fontSize: '22px', color: '#012374' }}>Daily glucose pattern</h2>
            ...
            <div style={{ display: 'flex', gap: '7px', flexShrink: 0 }}>
              {[7, 30, 90].map(d => (
                <button key={d} onClick={() => setPeriod(d)} style={{
                  padding: '7px 15px', borderRadius: '999px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: 'none', fontFamily: 'inherit',
          ...
          {tir && (
            <div style={{ marginTop: '18px' }}>
              <TirBar low={tir.low} normal={tir.normal} high={tir.high} />
            </div>
          )}
        ...
        {featuredCard && (
          <div style={{ marginTop: '20px', background: '#012374', borderRadius: '22px', padding: '26px 28px', color: '#FFFDF9' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '9px', marginBottom: '12px' }}>
              ...
              <span style={{ fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7FD0D0', fontWeight: 700 }}>
                Pattern of the week
              </span>
            </div>
            <h2 className="font-serif-italic" style={{ fontSize: '24px', lineHeight: 1.25, maxWidth: '600px' }}>{featuredCard.title}</h2>
        ...
        {/* ── Insight cards (3-col desktop, 1-col mobile) ── */}
        {allCards.length > 1 && (
          <div style={{
            marginTop: '20px',
            display: 'grid',
        ...
        {/* Extra charts (meal comparison, daily pattern) — collapsed into accessible expanders */}
...
function PageHeader({ period, setPeriod, correlation }: { period: number; setPeriod: (p: number) => void; correlation: any }) {
  return (
    <div style={{ background: '#FFFDF9', borderBottom: '1px solid rgba(1,35,116,0.07)' }}>
      <div style={{ maxWidth: '980px', margin: '0 auto', padding: '18px 20px 14px' }}>
