...
const STRESS_LEVEL_KEYS: Record<number, string> = {
  1: 'veryEasy', 2: 'easy', 3: 'easy', 4: 'mild', 5: 'mild',
  6: 'some', 7: 'some', 8: 'heavy', 9: 'heavy', 10: 'aLot',
};

// ── Helpers ────────────────────────────────────────────────────────────────────

type MoodT = { justNow: string; minAgo: string; hoursAgo: string; yesterday: string; today: string };

function formatDate(dateStr: string, mt: MoodT, locale: string) {
  ...
  if (diffMins < 1) return mt.justNow;
  if (diffMins < 60) return mt.minAgo.replace('{m}', String(diffMins));
  if (diffHours < 24) return mt.hoursAgo.replace('{h}', String(diffHours));
  if (diffDays === 1) return mt.yesterday;
  return d.toLocaleDateString(locale, { month: 'short', day: 'numeric' });
}

function groupByDay(entries: MoodEntry[], mt: MoodT, locale: string) {
...
function EntryCard({ entry, web }: { entry: MoodEntry; web?: boolean }) {
  ...
  const stressKey = STRESS_LEVEL_KEYS[entry.stressLevel];
  const stressLabel = stressKey ? (t.moodLog.stressLevels as Record<string, string>)[stressKey] : '';
  const tags = [
    entry.notFeelingWell && t.moodLog.tagNotWell,
    entry.onPeriod && t.moodLog.tagPeriod,
    entry.feelingOverwhelmed && t.moodLog.tagOverwhelmed,
    entry.havingCravings && t.moodLog.tagCravings,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: '#16182A' }}>
            {(t.moodLog.moods as Record<string, string>)[entry.mood] ?? entry.mood}
          </span>
          <span style={{ fontSize: 12, color: 'rgba(22,24,42,0.45)', flexShrink: 0 }}>
            {formatDate(entry.recordedAt, t.moodLog, locale)}
          </span>
        </div>
        <div style={{ fontSize: 12, color: 'rgba(22,24,42,0.55)', marginTop: 2 }}>
          {t.moodLog.stress}: {stressLabel}
        </div>
        {tags.length > 0 && (
        ...
        {(entry.energyLevel || entry.hungerLevel || entry.fullnessLevel) && (
          <div style={{ display: 'flex', gap: 10, marginTop: 7, flexWrap: 'wrap' }}>
            {entry.energyLevel && <span style={{ fontSize: 11, color: 'rgba(22,24,42,0.55)' }}>{t.moodLog.energy}: {entry.energyLevel}/10</span>}
            {entry.hungerLevel && <span style={{ fontSize: 11, color: 'rgba(22,24,42,0.55)' }}>{t.moodLog.hunger}: {entry.hungerLevel}/10</span>}
            {entry.fullnessLevel && <span style={{ fontSize: 11, color: 'rgba(22,24,42,0.55)' }}>{t.moodLog.fullness}: {entry.fullnessLevel}/10</span>}
          </div>
        )}
        {/* Parsed JSON arrays */}
        {(() => {
...
export default function MoodLogPage() {
  ...
  const grouped = groupByDay(entries, t.moodLog, locale);
  ...
  const EmptyState = () => (
    <div style={{ padding: '52px 20px', textAlign: 'center', background: '#FFFDF9', borderRadius: 22, border: '1.5px dashed rgba(1,35,116,0.16)' }}>
      ...
      <p className="font-serif-italic" style={{ fontSize: 22, color: '#012374' }}>{t.moodLog.noCheckins}</p>
      ...
      <div className="lg:hidden mobile-page-pb" style={{ minHeight: '100vh', background: '#F7EFE1', fontFamily: "'DM Sans', sans-serif" }}>
        <div style={{ padding: '18px 20px 0', paddingTop: 'max(18px, env(safe-area-inset-top, 0px))' }}>
          ...
          <div style={{ marginTop: 10 }}>
            <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#C8932B', fontWeight: 700 }}>{t.moodLog.kicker}</div>
            <h1 className="font-serif-italic" style={{ fontSize: 23, color: '#012374', lineHeight: 1 }}>{t.moodLog.heading}</h1>
          </div>
        </div>
          ...
          <div style={{ marginTop: 26, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22, alignItems: 'start' }}>
            ...
            <div>
              <div style={{ background: '#FFFDF9', borderRadius: 22, border: '1px solid rgba(1,35,116,0.07)', padding: 24, boxShadow: '0 14px 30px -24px rgba(1,35,116,.3)' }}>
                <div className="font-serif-italic" style={{ fontSize: 23, color: '#012374', marginBottom: 14 }}>{t.moodLog.recentCheckins}</div>
                <LogGroups web />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
              <div style={{ background: '#FFFDF9', borderRadius: 22, border: '1px solid rgba(1,35,116,0.07)', padding: 24, boxShadow: '0 14px 30px -24px rgba(1,35,116,.3)' }}>
                <div className="font-serif-italic" style={{ fontSize: 23, color: '#012374', marginBottom: 6 }}>{t.moodLog.weekAtGlance}</div>
                <p style={{ fontSize: 14, color: 'rgba(22,24,42,0.6)', lineHeight: 1.6 }}>
                  {entries.length === 0
                    ? t.moodLog.noCheckinsWeek
                    : entries.length === 1 ? t.moodLog.checkinLogged : t.moodLog.checkinsLogged.replace('{count}', String(entries.length))}
                </p>
                      ...
                      {top.map(([mood, count]) => {
                        ...
                        return (
                          <div key={mood} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#F7EFE1', borderRadius: 12, padding: '10px 14px' }}>
                            ...
                            <span style={{ fontSize: 14, fontWeight: 600, color: '#16182A' }}>{(t.moodLog.moods as Record<string, string>)[mood] ?? mood}</span>
              ...
              <div style={{ background: '#012374', borderRadius: 22, padding: 24, color: '#FFFDF9' }}>
                <div style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#C8932B', fontWeight: 700 }}>{t.moodLog.chatitaReminder}</div>
                <p style={{ fontSize: 16, lineHeight: 1.6, marginTop: 10, opacity: 0.92 }}>
                  {t.moodLog.reminderBody}
                </p>
              </div>
            </div>
          </div>
